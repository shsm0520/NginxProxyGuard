        ...
        {displayData.length === 0 ? (
          ...
          displayData.map((item, index) => (
            <div key={index} className="flex items-center gap-2">
              <span className="text-xs text-slate-600 w-36 truncate font-mono" title={item.label}>
                {item.label}
              </span>
...
export const StatusCodeChart = memo(function StatusCodeChart({ data }: StatusCodeChartProps) {
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-4">
      <h4 className="text-sm font-semibold text-slate-700 mb-3">Status Codes</h4>
      <div className="flex items-center gap-4">
        ...
        <div className="relative w-24 h-24">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <span className="text-xs font-bold text-slate-700">{total.toLocaleString()}</span>
          </div>
        </div>
        ...
        <div className="flex-1 space-y-1">
          {segments.slice(0, 6).map((seg, i) => (
            <div key={i} className="flex items-center gap-2 text-xs">
              ...
              <span className="text-slate-400 ml-auto">{seg.percentage.toFixed(1)}%</span>
            </div>
          ))}
        </div>
      </div>
    ...
    <div className="bg-slate-50 rounded-xl border border-slate-200 p-4">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        {topUserAgentsData.length > 0 && (
          <BarChart data={topUserAgentsData} title="Top User Agents" maxItems={8} />
        )}
        ...
        {logType === 'access' && statusCodeData.length > 0 && (
          <BarChart data={statusCodeData} title="Status Code Distribution" maxItems={10} />
        )}
      </div>
    </div>
  );
